    ...
    for k = 1:length(jpegFiles)
        ...
        makePly(mm, 'cropped_face.ply'); %visualize in MeshLab
        ptCloud_cropped = pcread('cropped_face.ply');
...
function inRng = inxrange(val, nose_pt)
        x_llmt = -70 + nose_pt(:,1); %change the range -70 to +70 as per your requirements
        x_ulmt = 70 - nose_pt(:,1);
...
function inRng = inyrange(val, nose_pt)
    y_llmt = -70 + nose_pt(:,2);        %change the range -70 to +70 as per your requirements
    y_ulmt = 70 - nose_pt(:,2);
